...
function mostrarMensajeReporte(mensaje, tipo = 'info') {
    if (window.Swal) {
    ...
    return window.mostrarNotificacionGlobalOriginal
        ? window.mostrarNotificacionGlobalOriginal(mensaje, tipo)
        : undefined;
}

if (!window.mostrarNotificacionGlobalOriginal && window.mostrarNotificacionGlobal) {
    window.mostrarNotificacionGlobalOriginal = window.mostrarNotificacionGlobal;
}

window.mostrarNotificacionGlobal = mostrarMensajeReporte;
...
async function cargarReporteUsuarios() {
     try {
         ...
         const response = await fetchWithAuth('/api/reportes/administrador/usuarios-general');
...
async function cargarReporteFichas() {
     try {
         ...
         if (container) {
             container.innerHTML = '<p style="color: #9fb3c8;">Cargando fichas...</p>';
         }
...
document.addEventListener('DOMContentLoaded', function() {
    const seccionReporteFichas = document.getElementById('reporte-fichas');
    if (seccionReporteFichas && seccionReporteFichas.classList.contains('active')) {
        cargarReporteFichas();
    }
});

function mostrarReporteFichas(fichas) {
     let html = '<table class="table"><thead><tr>';
...
function filtrarPorFicha() {
     ...
     mostrarReporteFichas(filtrados);
 }
...
async function cargarReporteInstructores() {
    try {
        ...
        const response = await fetchWithAuth('/api/reportes/administrador/instructores-especialidad');
...
async function cargarReporteTrimestres() {
    try {
        const container = document.getElementById('reporte-trimestres-list');
        if (container) {
            container.innerHTML = '<p style="color: #9fb3c8;">Cargando trimestres...</p>';
        }
        const response = await fetchWithAuth('/api/reportes/administrador/trimestres-ficha');
        ...
        mostrarReporteTrimestres(trimestres);
...
function mostrarReporteTrimestres(trimestres) {
    let html = '<table class="table"><thead><tr>';
...
function filtrarPorFichaTrimestres() {
    ...
    console.log('Mostrando:', filtrados.length, 'registros');
    mostrarReporteTrimestres(filtrados);
}
...
async function cargarReporteProyectos() {
    try {
        const response = await fetchWithAuth('/api/reportes/administrador/proyectos-gaes');
...
async function descargarReporteUsuariosPDF() {
     try {
         const response = await fetchWithAuth('/api/reportes/descargar/usuarios/pdf');
...
async function descargarReporteUsuariosExcel() {
     try {
         ...
         const response = await fetchWithAuth('/api/reportes/administrador/descargar-usuarios-excel', {
             method: 'POST',
             headers: {
...
async function descargarReporteFichasPDF() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/fichas/pdf');
        if (!response.ok) throw new Error('Error al descargar PDF');
...
async function descargarReporteFichasExcel() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/fichas/excel');
...
async function descargarReporteInstructoresPDF() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/instructores/pdf');
        if (!response.ok) throw new Error('Error al descargar PDF');
...
async function descargarReporteInstructoresExcel() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/instructores/excel');
        if (!response.ok) throw new Error('Error al descargar Excel');
...
async function descargarReporteTrimestresPDF() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/trimestres/pdf');
        if (!response.ok) throw new Error('Error al descargar PDF');
...
async function descargarReporteTrimestresExcel() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/trimestres/excel');
        if (!response.ok) throw new Error('Error al descargar Excel');
...
async function descargarReporteProyectosPDF() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/proyectos/pdf');
        if (!response.ok) throw new Error('Error al descargar PDF');
...
async function descargarReporteProyectosExcel() {
    try {
        const response = await fetchWithAuth('/api/reportes/descargar/proyectos/excel');
        if (!response.ok) throw new Error('Error al descargar Excel');
