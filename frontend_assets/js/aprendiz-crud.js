...
function mostrarAprendices(aprendices) {
    ...
    aprendices.forEach(aprendiz => {
        const usuario =
            window.usuariosListaCompleta.find(u => u.id === aprendiz.usuario?.id) ||
            aprendiz.usuario ||
            null;
        const ficha =
            window.fichasListaCompleta.find(f => f.id === aprendiz.ficha?.id) ||
            aprendiz.ficha ||
            null;
        ...
        row.innerHTML = `
             <td>${aprendiz.id}</td>
             <td>${usuario ? `${usuario.nombre || ''} ${usuario.apellido || ''}`.trim() : 'N/A'}</td>
             <td>${ficha ? (ficha.codigoFicha || ficha.codigo || 'N/A') : 'N/A'}</td>
...
function crearAprendiz(event) {
    ...
    fetchWithAuth(`${window.API_APRENDICES}/por-ids`, {
        method: 'POST',
        body: JSON.stringify(nuevoAprendiz)
    })
    .then(response => {
        if (!response) {
            throw new Error('No se pudo completar la peticion autenticada');
        }
...
function actualizarAprendiz(event) {
    ...
    fetchWithAuth(`${window.API_APRENDICES}/${id}`, {
        method: 'PUT',
...
// ===== DESACTIVAR APRENDIZ =====
async function desactivarAprendiz(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas desactivar este aprendiz?', 'Desactivar aprendiz');
        if (!confirmado) return;
    } else if (!confirm('Estas seguro de que deseas desactivar este aprendiz?')) {
        return;
    }
    
    fetchWithAuth(`${window.API_APRENDICES}/${id}/desactivar`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' }
    })
    .then(response => {
...
// ===== ACTIVAR APRENDIZ =====
async function activarAprendiz(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas activar este aprendiz?', 'Activar aprendiz');
        if (!confirmado) return;
    } else if (!confirm('Estas seguro de que deseas activar este aprendiz?')) {
        return;
    }
    
    fetchWithAuth(`${window.API_APRENDICES}/${id}/activar`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' }
    })
    .then(response => {
