...
function getGaesArrayAprendiz() {
    if (!aprendizData.gaes) return [];
    return Array.isArray(aprendizData.gaes) ? aprendizData.gaes : [aprendizData.gaes];
}

function getGaesPrincipalAprendiz() {
    return getGaesArrayAprendiz()[0] || null;
}

function getNotificacionesStorageKey() {
    const aprendizId = aprendizData?.aprendiz?.id || 'anon';
    return `aprendiz-notificaciones-${aprendizId}`;
}

function leerNotificacionesGuardadas() {
    try {
        const raw = localStorage.getItem(getNotificacionesStorageKey());
...
function guardarNotificaciones(notificaciones) {
    localStorage.setItem(getNotificacionesStorageKey(), JSON.stringify(notificaciones));
}

function construirNotificacionesDesdeEvaluaciones() {
    const evaluaciones = Array.isArray(aprendizData.evaluaciones) ? aprendizData.evaluaciones : [];
    return evaluaciones.map(evaluacion => {
        const tipo = evaluacion.tipo === 'GAES' ? 'GAES' : 'INDIVIDUAL';
        const entregable = evaluacion.entregableNombre || 'entregable';
        return {
            id: `eval-${evaluacion.id}`,
            evaluacionId: evaluacion.id,
            tipo,
            titulo: tipo === 'GAES' ? 'Calificado en GAES' : 'Calificado individual',
            mensaje: tipo === 'GAES'
                ? `Tu GAES fue calificado en ${entregable}.`
                : `Recibiste una calificacion individual en ${entregable}.`,
            fecha: evaluacion.fecha || new Date().toISOString().split('T')[0],
        };
    });
}

function sincronizarNotificacionesEvaluaciones() {
    const evaluaciones = Array.isArray(aprendizData.evaluaciones) ? aprendizData.evaluaciones : [];
    const notificaciones = leerNotificacionesGuardadas();
    const idsExistentes = new Set(notificaciones.map(item => item.evaluacionId));
    let nuevas = 0;

    evaluaciones.forEach(evaluacion => {
        if (idsExistentes.has(evaluacion.id)) return;
    ...
    guardarNotificaciones(notificaciones.slice(0, 50));

    if (nuevas > 0 && typeof mostrarNotificacionGlobal === 'function') {
        mostrarNotificacionGlobal(
            nuevas === 1 ? 'Tienes una nueva notificacion de calificacion' : `Tienes ${nuevas} nuevas notificaciones de calificacion`,
            'success'
        );
    }
}
...
function renderNotificaciones() {
    const recientes = document.getElementById('notificaciones-list');
    const completas = document.getElementById('notificaciones-full-list');
    const guardadas = leerNotificacionesGuardadas();
    const desdeEvaluaciones = construirNotificacionesDesdeEvaluaciones();
    const mapa = new Map();

    [...guardadas, ...desdeEvaluaciones].forEach(item => {
    ...
    const notificaciones = Array.from(mapa.values()).sort((a, b) => {
        const fechaA = a.fecha ? new Date(a.fecha).getTime() : 0;
        const fechaB = b.fecha ? new Date(b.fecha).getTime() : 0;
        return fechaB - fechaA;
    });

    const htmlVacio = '<p class="empty-state">No tienes notificaciones aun</p>';
    const renderItem = (item, corto = false) => `
        <div class="card" style="margin-bottom: 12px;">
            <h3>${item.titulo}</h3>
            ${corto ? '' : `<p><strong>Tipo:</strong> ${item.tipo}</p>`}
            <p>${item.mensaje}</p>
            ${corto ? '' : `<p><small>${item.fecha ? new Date(item.fecha).toLocaleDateString() : 'N/A'}</small></p>`}
        </div>
    `;

    if (recientes) {
        recientes.innerHTML = notificaciones.length > 0
            ? notificaciones.slice(0, 5).map(item => renderItem(item, true)).join('')
            : htmlVacio;
    }

    if (completas) {
        completas.innerHTML = notificaciones.length > 0
            ? notificaciones.map(item => renderItem(item)).join('')
            : htmlVacio;
    }

    if (notificaciones.length > 0 && guardadas.length === 0) {
        guardarNotificaciones(notificaciones.slice(0, 50));
    }
}
...
function loadAprendizData() {
    ...
    .then(([ficha, gaes, proyectos, evaluaciones]) => {
        ...
        if (ficha) {
            const fichasActuales = Array.isArray(window.fichasListaCompleta) ? window.fichasListaCompleta : [];
            const yaExiste = fichasActuales.some(item => item.id === ficha.id);
            if (!yaExiste) {
                window.fichasListaCompleta = [...fichasActuales, ficha];
            }
        }
        aprendizData.gaes = gaes;
        aprendizData.proyectos = proyectos || [];
        aprendizData.evaluaciones = evaluaciones || [];
        sincronizarNotificacionesEvaluaciones();
...
function updateDashboard() {
    ...
    if (aprendizData.gaes) {
        const gaesArray = getGaesArrayAprendiz();
...
function updateProyectosList() {
 ...
 function cargarTrimestresEnModal() {
       fetchWithAuth(`${API_BASE}/trimestres`)
           .then(response => {
           ...
           .then(trimestres => {
               ...
               const fichaId = aprendizData?.ficha?.id || aprendizData?.fichaId || null;
  ...
  function editarProyecto(id) {
        ...
        if (proyecto) {
          ...
          document.getElementById('proyecto-gaes').value = proyecto.gaesId || getGaesPrincipalAprendiz()?.id || '';
...
function abrirRegistrarProyecto() {
     ...
     if (gaesArray.length > 0) {
         gaesArray.forEach((gaes, index) => {
             const option = document.createElement('option');
             option.value = gaes.id;
             option.textContent = gaes.nombre;
             option.selected = index === 0;
             selectGaes.appendChild(option);
         });
         selectGaes.disabled = false;
     } else {
...
function resetFormularioEntregable() {
    const form = document.getElementById('form-subir-entregable');
    if (form) form.reset();

    const titulo = document.getElementById('modal-entregable-titulo');
    if (titulo) titulo.textContent = 'Subir Entregable';

    const submitBtn = document.getElementById('entregable-submit-btn');
    if (submitBtn) submitBtn.textContent = 'Enviar Entregable';

    const entregableIdInput = document.getElementById('entregable-id');
    if (entregableIdInput) entregableIdInput.value = '';
...
function updateEntregablesList() {
    ...
    fetchWithAuth(`${API_BASE}/entregables`)
        ...
        .then(entregables => {
            ...
            entregablesCache = Array.isArray(entregables) ? entregables : [];
            ...
            aprendizData.proyectos.forEach(proyecto => {
                const proyectoIdActual = Number(proyecto.id || 0);
                // Encontrar entregables de este proyecto
                const entregablesProyecto = entregables.filter(e => Number(e.proyectoId || 0) === proyectoIdActual);
                ...
                if (entregablesProyecto.length === 0) {
                    ...
                    entregablesProyecto.forEach((entregable, index) => {
                        const aprendizIdActual = Number(aprendizData?.aprendiz?.id || 0);
                        const aprendizIdEntregable = Number(entregable.aprendizId || 0);
                        const puedeEditar = aprendizIdActual > 0 && aprendizIdEntregable === aprendizIdActual;
                        const editarBtn = puedeEditar
                            ? `<button class="btn-secondary btn-sm" onclick="abrirEditarEntregable(${entregable.id})">Editar</button>`
                            : '';
                        const archivoBtn = (entregable.archivo || entregable.url)
                            ? `<button class="btn-secondary btn-sm" onclick="descargarEntregable(${entregable.id}, '${entregable.nombreArchivo || entregable.nombre || 'entregable'}')">📥 Descargar</button>`
...
function abrirEntregable(proyectoId) {
    resetFormularioEntregable();
    document.getElementById('entregable-proyecto').value = proyectoId;
    sincronizarTrimestresEntregable();
    openModal('modal-subir-entregable');
}

function abrirEditarEntregable(entregableId) {
    const entregable = entregablesCache.find(item => Number(item.id || 0) === Number(entregableId || 0));
    if (!entregable) {
        alert('No se encontro el entregable para editar.');
        return;
    }
    ...
    const titulo = document.getElementById('modal-entregable-titulo');
    if (titulo) titulo.textContent = 'Editar Entregable';

    const submitBtn = document.getElementById('entregable-submit-btn');
    if (submitBtn) submitBtn.textContent = 'Actualizar Entregable';

    const entregableIdInput = document.getElementById('entregable-id');
    if (entregableIdInput) entregableIdInput.value = entregable.id;

    document.getElementById('entregable-proyecto').value = entregable.proyectoId || '';
    document.getElementById('entregable-nombre').value = entregable.nombre || '';
    document.getElementById('entregable-descripcion').value = entregable.descripcion || '';
    document.getElementById('entregable-url').value = entregable.url || '';

    sincronizarTrimestresEntregable();
    if (entregable.trimestreId) {
        document.getElementById('entregable-trimestre').value = entregable.trimestreId;
    }
    ...
    if (entregable.url) {
        ...
        activarModoEntrega('archivo');
    }

    openModal('modal-subir-entregable');
}

function sincronizarTrimestresEntregable() {
    const selectProyecto = document.getElementById('proyecto-trimestre');
    const selectEntregable = document.getElementById('entregable-trimestre');

    if (!selectEntregable) return;

    selectEntregable.innerHTML = '<option value="">Elige un trimestre</option>';

    if (!selectProyecto) return;

    Array.from(selectProyecto.options)
        .filter(option => option.value)
        .forEach(option => {
            const nuevaOpcion = document.createElement('option');
            nuevaOpcion.value = option.value;
            nuevaOpcion.textContent = option.textContent;
            selectEntregable.appendChild(nuevaOpcion);
        });
}
...
async function subirEntregable(event) {
    ...
    const trimestreId = parseInt(document.getElementById('entregable-trimestre').value);
    ...
    if (!nombre || !trimestreId) {
        alert('Por favor ingresa el nombre del entregable y el trimestre');
        return;
    }
    ...
    try {
         const isEdit = Boolean(entregableId);
         console.log(isEdit ? 'Actualizando entregable...' : 'Subiendo entregable...');
         const response = await fetchWithAuth(
             isEdit ? `${API_BASE}/entregables/${entregableId}` : `${API_BASE}/entregables`,
             {
        ...
        if (response.ok) {
            ...
            console.log(isEdit ? 'Entregable actualizado correctamente:' : 'Entregable subido correctamente:', data);
            alert(isEdit ? 'Entregable actualizado correctamente' : 'Entregable subido correctamente');
            ...
            alert((entregableId ? 'Error al actualizar el entregable: ' : 'Error al subir el entregable: ') + error);
        }
    } catch (error) {
        console.error('Error:', error);
        alert((entregableId ? 'Error al actualizar el entregable: ' : 'Error al subir el entregable: ') + error.message);
    }
}
...
function updateCalificacionesList() {
    ...
    aprendizData.evaluaciones.forEach(evaluacion => {
        ...
        card.innerHTML = `
            <h3>Calificación</h3>
            <p><strong>Entregable:</strong> ${evaluacion.entregableNombre || 'N/A'}</p>
...
function closeModal(modalId) {
    ...
    if (modalId === 'modal-subir-entregable') {
        resetFormularioEntregable();
    }
}
...
async function logout() {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Deseas cerrar sesion?', 'Cerrar sesion');
        if (!confirmado) {
            return;
        }
    }
...
function actualizarEstadoBotonSubirEntregable(habilitado) {
    const boton = document.querySelector('#entregar-entregables .filters .btn-primary');
    if (!boton) return;

    boton.disabled = !habilitado;
    boton.style.opacity = habilitado ? '1' : '0.6';
    boton.style.pointerEvents = habilitado ? 'auto' : 'none';
    boton.title = habilitado ? '' : 'Espera a que el instructor cree un entregable';
}

function limpiarVistaEntregablesVacia() {
    const tbody = document.getElementById('entregables-tbody');
    ...
    filas.forEach(fila => {
        ...
        if (texto.includes('sin entregables subidos')) {
            fila.remove();
        }
    });

    const filasRestantes = Array.from(tbody.querySelectorAll('tr'));
    const hayEntregables = filasRestantes.some(fila => !fila.querySelector('.no-data'));

    if (!hayEntregables) {
        tbody.innerHTML = '<tr><td colspan="5" class="no-data">Aun no hay entregables creados por el instructor.</td></tr>';
        actualizarEstadoBotonSubirEntregable(false);
        return true;
    }

    actualizarEstadoBotonSubirEntregable(true);
    return true;
}

function programarLimpiezaVistaEntregables(intentos = 0) {
    window.setTimeout(() => {
        const procesado = limpiarVistaEntregablesVacia();
        if (!procesado && intentos < 10) {
            programarLimpiezaVistaEntregables(intentos + 1);
        }
    }, 150);
}

const updateEntregablesListOriginal = updateEntregablesList;
updateEntregablesList = function() {
    updateEntregablesListOriginal();
    programarLimpiezaVistaEntregables();
};
