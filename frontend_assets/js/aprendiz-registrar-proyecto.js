...
function subirEntregable(event) {
    ...
    const apiEntregables = window.API_ENTREGABLES || '/api/entregables';
    
    const proyectoId = document.getElementById('entregable-proyecto')?.value;
    const trimestreId = document.getElementById('entregable-trimestre')?.value;
    ...
    if (!proyectoId || !nombre || !trimestreId) {
        mostrarNotificacionGlobal('Por favor completa proyecto, trimestre y nombre', 'warning');
        return;
    }
    ...
    // Si es por archivo
        ...
        fetchPostFormData(apiEntregables, formData)
        .then(response => {
