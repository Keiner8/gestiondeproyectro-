...
function actualizarEvaluacion(event) {
    ...
    fetchWithAuth(`${window.API_EVALUACIONES}/${id}`, {
     ...
     .then(data => {
        ...
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas eliminar este reporte?', 'Eliminar reporte');
        if (!confirmado) return;
    } else if (!confirm('Estas seguro de que deseas eliminar este reporte?')) {
        return;
    }
