...
function normalizarFicha(ficha) {
    return {
        ...ficha,
        codigoFicha: ficha.codigoFicha || ficha.codigo || ficha.codigo_ficha || '',
        programaFormacion: ficha.programaFormacion || ficha.programa || ficha.programa_formacion || '',
        nivel: ficha.nivel || ficha.nivel_formacion || '',
        fechaInicio: ficha.fechaInicio || ficha.fecha_inicio || '',
        fechaFin: ficha.fechaFin || ficha.fecha_fin || ''
    };
}
...
function cargarFichas() {
    fetchWithAuth(window.API_FICHAS)
        ...
        .then(data => {
            window.fichasListaCompleta = data.map(normalizarFicha);
            mostrarFichas(window.fichasListaCompleta);
        })
        .catch(error => {
...
function mostrarFichas(fichas) {
    ...
    if (fichas.length === 0) {
        tbody.innerHTML = '<tr><td colspan="10" class="no-data">No hay fichas registradas</td></tr>';
        return;
    }
    
    fichas.forEach(fichaOriginal => {
        const ficha = normalizarFicha(fichaOriginal);
        ...
        row.innerHTML = `
            <td>${ficha.id}</td>
            <td>${ficha.codigoFicha || 'N/A'}</td>
            <td>${ficha.programaFormacion || 'N/A'}</td>
            <td>${ficha.nivel || 'N/A'}</td>
...
function aplicarFiltrosFichas() {
    ...
    let fichasFiltradas = window.fichasListaCompleta.filter(ficha => {
        const codigo = (ficha.codigoFicha || '').toLowerCase();
        const programa = (ficha.programaFormacion || '').toLowerCase();
        const matchBusqueda = codigo.includes(searchTerm) || programa.includes(searchTerm);
...
function abrirEditarFicha(id) {
    ...
    document.getElementById('ficha-nivel-edit').value = ficha.nivel || '';
...
function actualizarFicha(event) {
    ...
    const nivel = document.getElementById('ficha-nivel-edit').value;
    ...
    if (!codigo || !programa || !nivel || !jornada || !modalidad || !fechaInicio || !fechaFin) {
        mostrarNotificacionFicha('Por favor completa todos los campos obligatorios', 'warning');
        return;
    }
...
Estado: ${ficha.estado}
    ...
    if (window.mostrarAlertaGlobal) {
        await window.mostrarAlertaGlobal(mensaje, 'info', 'Detalle de la ficha');
        return;
    }
    alert(mensaje);
}
...
async function eliminarFicha(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas eliminar esta ficha?', 'Eliminar ficha');
        if (!confirmado) return;
    } else if (!confirm('Estas seguro de que deseas eliminar esta ficha?')) {
        return;
    }
