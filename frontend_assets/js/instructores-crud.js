...
function actualizarInstructor(event) {
    ...
    fetchWithAuth(`${window.API_INSTRUCTORES}/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
async function cambiarEstadoInstructor(id) {
     const instructor = window.instructoresListaCompleta.find(i => i.id === id);
     if (!instructor) return;
     ...
     if (window.confirmarAccion) {
         const confirmado = await window.confirmarAccion(mensaje, 'Confirmar accion');
         if (!confirmado) {
             return;
         }
...
async function eliminarInstructor(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas eliminar este instructor?', 'Eliminar instructor');
        if (!confirmado) return;
    } else if (!confirm('Estas seguro de que deseas eliminar este instructor?')) {
        return;
    }
