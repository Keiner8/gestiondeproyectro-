...
function aplicarFiltrosRoles() {
    if (!document.getElementById('roles-search')) return;
...
async function eliminarRol(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas eliminar este rol?', 'Eliminar rol');
        if (!confirmado) return;
    } else if (!confirm('Estas seguro de que deseas eliminar este rol?')) {
        return;
    }
