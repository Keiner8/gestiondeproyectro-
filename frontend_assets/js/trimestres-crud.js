...
// ===== ELIMINAR TRIMESTRE =====
async function eliminarTrimestre(id) {
     if (window.confirmarAccion) {
         const confirmado = await window.confirmarAccion('Estas seguro de que deseas eliminar este trimestre?', 'Eliminar trimestre');
         if (!confirmado) return;
     } else if (!confirm('Estas seguro de que deseas eliminar este trimestre?')) {
         return;
     }
