...
function obtenerNombreRol(usuario) {
    ...
    return rol?.nombreRol || usuario.rol?.nombreRol || 'Sin asignar';
}
...
function mostrarUsuarios(usuarios) {
    ...
    usuarios.forEach(usuario => {
        const rolNombre = obtenerNombreRol(usuario);
...
window.verUsuario = async function(id) {
    ...
    const mensaje = `ID: ${usuario.id}\nNombre: ${usuario.nombre} ${usuario.apellido}\nCorreo: ${usuario.correo}\nDocumento: ${tipoDocFormatted} - ${usuario.numeroDocumento}\nRol: ${rolNombre}\nEstado: ${usuario.estado}`;

    if (window.mostrarAlertaGlobal) {
        await window.mostrarAlertaGlobal(mensaje, 'info', 'Detalle del usuario');
        return;
    }
    alert(mensaje);
};
...
window.desactivarUsuario = async function(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas desactivar este usuario?', 'Desactivar usuario');
...
window.activarUsuario = async function(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas activar este usuario?', 'Activar usuario');
        if (!confirmado) return;
    }
...
window.eliminarUsuario = async function(id) {
    if (window.confirmarAccion) {
        const confirmado = await window.confirmarAccion('Estas seguro de que deseas eliminar este usuario?', 'Eliminar usuario');
        if (!confirmado) return;
    }
